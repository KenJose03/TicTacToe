...
# black game board, dividing board
# into 9 parts
...
initiating_window = pg.image.load("titletictac.jpg")
x_img = pg.image.load("XImage.jpg")
y_img = pg.image.load("OImage.jpg")
...
lead = open("leaderboard.bin","wb")
lst = [["x",0],["o",0]]
pickle.dump(lst,lead)
lead.close()
...
def leaderboard():
    global winner
    lead = open("leaderboard.bin","rb")
    lead.seek(0)
    dat = pickle.load(lead)
    if winner == "x":
        dat[0][1] += 1
    elif winner == 'o':
        dat[1][1] += 1
    lead.close()
    lead = open("leaderboard.bin", "wb")
    pickle.dump(dat,lead)
    lead.close()
    print("----------LEADERBOARD-----------")
    print("----X---------------------O-----")
    print("---",dat[0][1],"--------------------",dat[1][1],"-----")
